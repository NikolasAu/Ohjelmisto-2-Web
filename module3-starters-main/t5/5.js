...
document.querySelector('#pictures').classList.toggle('card')

const arcticle = document.querySelector('#pictures');
...
for (let pic of picArray) {
  ...
  const fig = document.createTextNode('Caption 9.');

  const figure = document.createElement('figure')
  figure.appendChild(i);
  figure.appendChild(fig);

  const h2 = document.createTextNode('Title 9');

  const p = document.createTextNode(      'Praesent eget cursus urna. Ut rhoncus interdum dolor non tincidunt. Sed vehicula consequat facilisis. Pellentesque pulvinar sem nisl, ac vestibulum erat rhoncus id. Phasellus imperdiet nunc tincidunt molestie vestibulum. Donec dictum suscipit nibh. Sed vel velit ante. Aenean quis viverra magna. ',
);

  arcticle.appendChild(h2);
  arcticle.appendChild(figure);
  arcticle.appendChild(p);
